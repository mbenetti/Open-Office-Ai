...
import { buildDocContext, blockRangePositions, countWords } from '../src/renderer/ai/protocol'
...
describe('selection payload', () => {
  it('lists every block covered by the selection under User selection:', () => {
    const editor = createEditor(fixture())
    // Select blocks 1 and 2 (the paragraph and the following heading).
    const { from, to } = blockRangePositions(editor, 1, 2)
    editor.commands.setTextSelection({ from, to })

    const context = buildDocContext(editor)
    expect(context).toContain('User selection:')
    expect(context).toContain("Block 1 'GenSpark is an AI office suite.'")
    expect(context).toContain("Block 2 'Risk Notes'")
  })
})
