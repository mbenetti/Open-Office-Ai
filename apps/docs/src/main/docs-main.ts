...
function userDataPath(...parts: string[]): string {
  const base = app?.getPath?.('appData') ?? app?.getPath?.('home') ?? process.cwd()
  return join(base, 'GenOffice', ...parts)
}
...
export function registerAiIpc(): void {
  ...
  ipcMain.handle('ai:stream', async (event, request: AiStreamRequest) => {
    ...
    const activeProv = freshSettings.provider
    const freshKey = freshSettings.providers?.[activeProv]?.apiKey
    const settings = freshKey || activeProv === 'custom' ? freshSettings : request.settings
    ...
    if (!config?.apiKey && provider !== 'custom' && provider !== 'ollama' && provider !== 'vllm') {
      send({
        requestId,
        type: 'error',
...
/** Deletes a document's chat history (called by the shell when a file is deleted/trashed). */
export function clearProjectChatForFile(filePath: string): void {
  try {
    getProjectStore().clearChatForFile(filePath)
  } catch (err) {
    console.warn('[project-store] clearChatForFile failed:', err)
  }
}
...
export function registerProjectIpc(): void {
  ...
  /** Create a brand-new conversation for the file (the "new chat" header action) */
  ipcMain.handle(
    'project:newChat',
    (event, args: { filePath?: string | null; sessionId?: string }) => {
      const store = getProjectStore()
      store.ensureDefaultProject()
      let resolvedPath = args.filePath ?? null
      if (!resolvedPath && args.sessionId && sessionPathResolver) {
        resolvedPath = sessionPathResolver(event.sender.id, args.sessionId)
      }
      if (!resolvedPath) {
      ...
      return store.createChatForFile(resolvedPath)
    },
  )

  /** List the file's conversations for the chat picker */
  ipcMain.handle('project:listChats', (event, args: { filePath?: string | null; sessionId?: string }) => {
    const store = getProjectStore()
    let resolvedPath = args.filePath ?? null
    if (!resolvedPath && args.sessionId && sessionPathResolver) {
      resolvedPath = sessionPathResolver(event.sender.id, args.sessionId)
  ...
  /** Switch the file's current chat to a previous conversation */
  ipcMain.handle(
    'project:selectChat',
    (event, args: { filePath?: string | null; sessionId?: string; chatId: string }) => {
      ...
      if (!resolvedPath || typeof args.chatId !== 'string') return null
      return store.selectChatForFile(resolvedPath, args.chatId)
    },
  )
  ...
  /** Clear a chat's history (delete the JSONL for the document) */
  ipcMain.handle('project:clearChat', (_event, args: { projectId: string; chatId: string }) => {
    ...
    getProjectStore().clearChat(args.projectId, args.chatId)
  })
