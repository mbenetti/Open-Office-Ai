...
export const COMMANDS_GUIDE = [
  ...
  '- Protected blocks such as images cannot be modified with style commands (they are skipped); to fill form fields or update text in an existing table, use update_table_cells to modify specific cells by row and column index. Do NOT use replace_blocks on existing tables unless the user explicitly asks to delete or recreate the table layout. Using update_table_cells preserves the original table styling, borders, shading, cell padding, and formatting 100% byte-for-byte.',
  '',
...
export function buildDocContext(editor: Editor): string {
  ...
  const userSelectionLines: string[] = []
  if (scope.isRange) {
    userSelectionLines.push('User selection:')
    const doc = editor.state.doc
    for (let i = scope.startIndex; i <= scope.endIndex && i < doc.childCount; i++) {
      const child = doc.child(i)
      const text = child.textContent ?? ''
      userSelectionLines.push(`Block ${i} '${text.replaceAll("'", "\\'")}'`)
    }
  }
  const userSelectionText = userSelectionLines.length > 0 ? userSelectionLines.join('\n') : ''
