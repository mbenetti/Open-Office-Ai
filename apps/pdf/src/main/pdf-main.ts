...
function registerPdfIpc(): void {
  ...
  // Knowledge Base chat (KB list + filtered search) shares the shell's store
  // and IPC channels, same as the docs/sheets/slides main processes.
  registerKnowledgeIpc()
...
// In shell mode the shell main registers project:* centrally (docs-main's
// registerProjectIpc), so this is only registered for standalone runs.

let pdfProjectStore: ProjectStore | null = null
let pdfProjectIpcRegistered = false

function getPdfProjectStore(): ProjectStore {
  if (!pdfProjectStore) pdfProjectStore = new ProjectStore(app.getPath('userData'))
  return pdfProjectStore
}

function registerProjectIpc(): void {
  if (pdfProjectIpcRegistered) return
  pdfProjectIpcRegistered = true

  ipcMain.handle('project:resolveChat', (_event, args: { filePath?: string | null }) => {
    const store = getPdfProjectStore()
    store.ensureDefaultProject()
    if (!args?.filePath) {
      return { projectId: 'default', chatId: `unsaved-${Date.now()}` }
    }
    return store.resolveChatForFile(args.filePath)
  })

  ipcMain.handle('project:newChat', (_event, args: { filePath?: string | null }) => {
    const store = getPdfProjectStore()
    store.ensureDefaultProject()
    if (!args?.filePath) return { projectId: 'default', chatId: `unsaved-${Date.now()}` }
    return store.createChatForFile(args.filePath)
  })

  ipcMain.handle('project:listChats', (_event, args: { filePath?: string | null }) => {
    if (!args?.filePath) return []
    return getPdfProjectStore().listChatsForFile(args.filePath)
  })

  ipcMain.handle(
    'project:selectChat',
    (_event, args: { filePath?: string | null; chatId: string }) => {
      if (!args?.filePath || typeof args.chatId !== 'string') return null
      return getPdfProjectStore().selectChatForFile(args.filePath, args.chatId)
    },
  )
  ...
  ipcMain.handle(
    ...
    (
      ...
      args: {
        ...
        role: 'user' | 'assistant'
        text: string
        tools?: Array<{ name: string; summary: string; isError?: boolean; input?: string; output?: string }>
      ...
      getPdfProjectStore().appendChatMessage(args.projectId, args.chatId, msg)
    },
  )
  ...
  ipcMain.handle(
    ...
    (_event, args: { projectId: string; chatId: string; limit?: number }) => {
      return getPdfProjectStore().loadChat(args.projectId, args.chatId, args.limit ?? 200)
    },
  )

  ipcMain.handle('project:clearChat', (_event, args: { projectId: string; chatId: string }) => {
    if (typeof args?.projectId !== 'string' || typeof args?.chatId !== 'string') return
    getPdfProjectStore().clearChat(args.projectId, args.chatId)
  })

  ipcMain.handle(
    'project:rebindChat',
    (_event, args: { projectId: string; tempChatId: string; newFilePath?: string }) => {
      const store = getPdfProjectStore()
      if (args.newFilePath) {
        return store.rebindChatToFile(args.projectId, args.tempChatId, args.newFilePath)
      }
      return { projectId: args.projectId, chatId: args.tempChatId }
    },
  )
}
