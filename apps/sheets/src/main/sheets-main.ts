...
export function registerSheetsAiIpc(): void {
  ...
  ipcMain.handle(IPC_CHANNELS.aiChat, async (event, input: unknown) => {
    ...
    if (!config) return { ok: false, error: 'No configuration for provider' }
  ...
  ipcMain.handle(IPC_CHANNELS.aiStream, async (event, input: unknown) => {
    const entry = sessionFor(event)
    const rawRequest = aiStreamRequestSchema.parse(input)
    ...
    const settings = freshKey || activeProv === 'custom' ? freshSettings : rawRequest.settings

    const request = { ...rawRequest, settings }
    ...
    if (!config) {
      send({ requestId, type: 'error', error: 'No configuration for provider' })
      return
    }
    if (!config.apiKey && provider !== 'custom' && provider !== 'ollama' && provider !== 'vllm') {
      send({
...
export function registerProjectIpc(): void {
  ...
  ipcMain.handle('project:newChat', (event, args: { filePath?: string | null; sessionId?: string }) => {
    const store = getSheetsProjectStore()
    store.ensureDefaultProject()
    ...
    if (!resolvedPath) return { projectId: 'default', chatId: `unsaved-${Date.now()}` }
  ...
  ipcMain.handle('project:listChats', (event, args: { filePath?: string | null; sessionId?: string }) => {
    ...
    return getSheetsProjectStore().listChatsForFile(resolvedPath)
  })
  ...
  ipcMain.handle(
    ...
    (event, args: { filePath?: string | null; sessionId?: string; chatId: string }) => {
      ...
      return getSheetsProjectStore().selectChatForFile(resolvedPath, args.chatId)
    },
  )
  ...
  ipcMain.handle('project:clearChat', (_event, args: { projectId: string; chatId: string }) => {
    ...
    getSheetsProjectStore().clearChat(args.projectId, args.chatId)
  })
