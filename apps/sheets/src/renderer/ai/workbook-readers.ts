...
export function getActiveSheetInfo(ctx: WorkbookReadContext): ActiveSheetInfo {
  const workbook = ctx.univerRef.current?.univerAPI.getActiveWorkbook()
  const selectedRanges = getSelectedRanges(workbook)
  const selection =
    selectedRanges
      .map((r) => (typeof r?.getA1Notation === 'function' ? r.getA1Notation() : ''))
      .filter(Boolean)
      .join(', ') || workbook?.getActiveRange()?.getA1Notation() || undefined
  const state = ctx.lazyWorkbookRef.current
  if (state) {
    ...
    return {
      ...
      selection,
      selectedCells: readSelectedCells(ctx),
...
function getSelectedRanges(workbook: any): any[] {
  if (!workbook) return []

  // 1. Try worksheet.getSelection() ranges
  try {
    const worksheet = workbook.getActiveSheet()
    if (worksheet) {
      if (typeof worksheet.getSelection === 'function') {
        const sel = worksheet.getSelection()
        if (sel) {
          // Current Univer facade: FSelection.getActiveRangeList() returns every
          // selected range (Ctrl/Cmd+click multi-selection), not just the active
          // cell; the older getRanges/getSelections shapes are kept as fallbacks.
          if (typeof sel.getActiveRangeList === 'function') {
            const ranges = sel.getActiveRangeList()
            if (Array.isArray(ranges) && ranges.length > 0) return ranges
          }
          if (typeof sel.getRanges === 'function') {
            const ranges = sel.getRanges()
            if (Array.isArray(ranges) && ranges.length > 0) return ranges
          }
          if (typeof sel.getSelections === 'function') {
            const selections = sel.getSelections()
  ...
  // 2. Try workbook.getActiveRanges() or workbook.getActiveRangeList()
  try {
    if (typeof workbook.getActiveRanges === 'function') {
      const ranges = workbook.getActiveRanges()
      if (Array.isArray(ranges) && ranges.length > 0) return ranges
    }
    if (typeof workbook.getActiveRangeList === 'function') {
      const list = workbook.getActiveRangeList()
      if (Array.isArray(list) && list.length > 0) return list
      const ranges = list?.getRanges?.()
      if (Array.isArray(ranges) && ranges.length > 0) return ranges
    }
  } catch {
    /* ignore fallback */
  ...
  // 3. Fallback to primary active range
  const primary = workbook.getActiveRange()
  return primary ? [primary] : []
}

export function readSelectedCells(
  ctx: WorkbookReadContext,
): { address: string; text: string }[] | undefined {
  const workbook = ctx.univerRef.current?.univerAPI.getActiveWorkbook()
  if (!workbook) return undefined
  const ranges = getSelectedRanges(workbook)
  ...
  for (const range of ranges) {
    if (addresses.length >= MAX_CELLS) break
    try {
      ...
      for (let r = 0; r < numRows && addresses.length < MAX_CELLS; r++) {
        for (let c = 0; c < numCols && addresses.length < MAX_CELLS; c++) {
          addresses.push(`${columnLabel(startCol + c)}${startRow + r + 1}`)
        }
      }
    } catch {
  ...
  const cellData = readCells(ctx, addresses)
  return addresses.map((addr) => {
    const data = cellData[addr]
