...
export interface ActiveSheetInfo {
  ...
  /** selected cell addresses with their string content for User_selection context */
  readonly selectedCells?: readonly { address: string; text: string }[] | undefined
...
export function buildWorkbookContext(deps: SheetsSkillDeps): string {
  ...
  if (info.selectedCells && info.selectedCells.length > 0) {
    lines.push('')
    lines.push('User selection:')
    for (const cell of info.selectedCells) {
      lines.push(`${cell.address} '${cell.text.replaceAll("'", "\\'")}'`)
    }
  }
  return lines.join('\n')
}
