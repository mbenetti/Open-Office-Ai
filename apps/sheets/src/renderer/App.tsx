...
import { AgentLoop, composeSkills, createKnowledgeSkill, createSkillsSkill, resolveSlashCommand, type AgentImage } from '@genoffice/agent-core'
...
export function App(): React.JSX.Element {
  ...
  useEffect(() => {
    window.desktopApi?.listKnowledgeBases?.().then((kbs) => {
  ...
  const [chatList, setChatList] = useState<
    Array<{ chatId: string; createdAt: string; preview?: string }>
  >([])
  /** chatId of the conversation currently shown (for the picker's active highlight) */
  ...
  useEffect(() => {
    window.desktopApi?.listSkills?.().then((list) => {
      if (Array.isArray(list)) setSkills(list)
    })
  }, [])

  const chatRefArgs = (): { filePath: null; sessionId?: string } => {
    const sessionId = workbookFile?.sessionId
    return sessionId !== undefined ? { filePath: null, sessionId } : { filePath: null }
  }
  ...
  if (!agentLoopRef.current) {
    agentLoopRef.current = new AgentLoop({
      ...
      skill: composeSkills('sheets+files+knowledge+skills', '', [
        ...
        createSkillsSkill(
          ...
          (name) => window.desktopApi?.getSkill?.(name) ?? Promise.resolve(null),
        ),
      ]),
  ...
  function handleClearHistory(): void {
    ...
    if (api?.resolveChat) {
      void api
        .resolveChat(chatRefArgs())
        .then((r) => {
          chatRefIdsRef.current = r
          setCurrentChatId(r.chatId)
          refreshChatList()
        })
        .catch(() => {
          /* silent */
  ...
  function handleSend(overrideInstruction?: string): void {
    const raw = (overrideInstruction ?? prompt).trim()
    if (!raw || aiBusy) return
    if (raw.startsWith('/') && !overrideInstruction) {
      void resolveSlashCommand(raw, (name) =>
        window.desktopApi?.getSkill?.(name) ?? Promise.resolve(null),
      )
        .then(({ instruction, applied }) => {
          setPrompt('')
          sendResolved(instruction, applied ? raw : instruction)
        })
        .catch(() => {
          setPrompt('')
          sendResolved(raw)
  ...
  function sendResolved(instruction: string, displayInstruction?: string): void {
    ...
    try {
      ...
      for (const change of stored.plan.cellChanges) {
        ...
        if (change.after.formula) {
          range.setFormula(change.after.formula)
        } else if (change.after.value === null) {
          range.clearContent()
        } else {
          ...
          const next: ICellData = { v: change.after.value, f: null, si: null }
          // Only completely unstyled cells inherit the document font (column
          // above, then row left). Anything the user or the file styled keeps
          // its own formatting — never override an existing font. The 'cell'
          // style is the cell's own (row/column/default composition excluded),
          // so a font coming from a row or column style still triggers
          // inheritance instead of being treated as an own style.
          const own = range.getCellStyleData('cell')
          if (own === null || Object.keys(own).length === 0) {
            const doc = docFontOf(worksheet, change.address)
            if (doc) next.s = doc
          }
  ...
  function openLazyWorkbook(opened: WorkbookFile): void {
    ...
    // Re-opening the same file (save / auto-save after an AI run) re-creates
    // the workbook unit, which resets every sheet's zoom to the open default.
    // Keep the user's per-sheet zoom when the same file on disk is reopened.
    const previousZoom = new Map<string, number>()
    if (previous && previous.file.path && previous.file.path === selected.path) {
      const workbook = univerRef.current?.univerAPI.getActiveWorkbook()
      if (workbook) {
        for (const ws of workbook.getSheets()) {
          const zoom = ws.getZoom()
          if (Number.isFinite(zoom) && zoom > 0) previousZoom.set(ws.getSheetId(), zoom)
        }
      }
    }
    if (previous) {
    ...
    if (runtime) {
      requestAnimationFrame(() => {
        const workbook = runtime.univerAPI.getActiveWorkbook()
        if (!workbook) return
        // Restore the zoom the user had set on each sheet before the unit was
        // recreated (save / auto-save reopen).
        for (const [sheetId, zoom] of previousZoom) {
          const ws = workbook.getSheetBySheetId(sheetId)
          if (ws && Math.abs(ws.getZoom() - zoom) > 1e-6) ws.zoom(zoom)
        }
