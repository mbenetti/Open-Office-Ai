...
/**
 * First existing font wins when a written cell has no font of its own: the
 * target cell itself, then the column above, then the row left. Coordinates
 * are 0-based — the scan must reach row 0 / column 0, the cells that most
 * often carry the document's header font.
 */
export function docFontOf(
  ws: UniverWorksheet,
  address: string,
): { ff?: string; fs?: number } | null {
  const { row, column } = parseAddress(address)
  for (let r = row - 1; r >= 0; r--) {
    const style = ws.getRange(r, column).getCellStyleData()
    if (style?.ff) {
      return { ff: style.ff, ...(typeof style.fs === 'number' ? { fs: style.fs } : {}) }
    }
  }
  for (let c = column - 1; c >= 0; c--) {
    const style = ws.getRange(row, c).getCellStyleData()
    if (style?.ff) {
      return { ff: style.ff, ...(typeof style.fs === 'number' ? { fs: style.fs } : {}) }
    }
  }
