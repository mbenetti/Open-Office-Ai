...
describe('buildWorkbookContext', () => {
  ...
  it('formats selected cells under User selection:', () => {
    const deps = fakeDeps({
      getActiveSheetInfo: () => ({
        mode: 'demo',
        sheetId: 'sheet-1',
        ...
        selectedCells: [
          { address: 'A1', text: 'This is the text from the cell' },
          { address: 'B1', text: 'This is other cell selected' },
          { address: 'C1', text: '' },
        ],
      }),
    })
    const text = buildWorkbookContext(deps)
    expect(text).toContain('User selection:')
    expect(text).toContain("A1 'This is the text from the cell'")
    expect(text).toContain("B1 'This is other cell selected'")
