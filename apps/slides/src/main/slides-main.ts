...
export function registerSlidesIpc(): void {
  ...
  ipcMain.handle(
    ...
    async (): Promise<{ ok: boolean; marker?: string; error?: string }> => {
      return { ok: false, error: 'cloud slide generation is disabled' }
    },
  )
...
export function registerProjectIpc(): void {
  ...
  ipcMain.handle('project:listChats', (_event, args: { filePath?: string | null }) => {
    ...
    return getSlidesProjectStore().listChatsForFile(args.filePath)
  })
  ...
  ipcMain.handle(
    ...
    (_event, args: { filePath?: string | null; chatId: string }) => {
      ...
      return getSlidesProjectStore().selectChatForFile(args.filePath, args.chatId)
    },
  )
  ...
  ipcMain.handle('project:clearChat', (_event, args: { projectId: string; chatId: string }) => {
    ...
    getSlidesProjectStore().clearChat(args.projectId, args.chatId)
  })
