...
const AI_SETTINGS_PATH = () => {
  ...
  return join(base, 'GenOffice', 'ai-settings.json')
}
...
export function registerSlidesOnlyAiIpc(): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      return { error: 'Image generation requires an active MCP server' }
    },
  )

  ipcMain.handle(
    'ai:analyze-media',
    async (_event, _op: { mediaUrls: string[]; requirements: string }) => {
      return { error: 'Media analysis requires an active MCP server' }
    },
  )
