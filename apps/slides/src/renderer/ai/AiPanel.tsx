...
import { Markdown, formatChatTimestamp } from '@genoffice/ui'
...
export function AiPanel({
  ...
  const skillToken =
    !skills || busy || !input.trim().startsWith('/')
      ? null
      : input.trim().slice(1).split(/\s+/)[0]!.toLowerCase()
  const skillMatches =
    skillToken === null
      ? []
      : (skillToken
          ? skills.filter((s) => s.name.toLowerCase().startsWith(skillToken))
          : skills
        ).slice(0, 6)
  ...
  const refreshChatList = () => {
    ...
    void api
      .listChats({ filePath: currentFilePath ?? null })
  ...
  if (!loopRef.current) {
    ...
    loopRef.current = new AgentLoop({
      ...
      skill: composeSkills('slides+files+knowledge+skills', '', [
        ...
        createSkillsSkill(
          ...
          (name) => (window as any).desktop?.getSkill?.(name) ?? Promise.resolve(null),
        ),
      ]),
  ...
  const newChat = () => {
    ...
    void api
      .newChat({ filePath: currentFilePath ?? null })
      .then((ids) => {
        chatRefIds.current = ids
        dismissClarify()
        qcAbortRef.current?.abort()
  ...
  const selectChat = (chatId: string) => {
    ...
    void api
      .selectChat({ filePath: currentFilePath ?? null, chatId })
      .then((ids) => {
        if (!ids) return null
        chatRefIds.current = ids
        dismissClarify()
  ...
  const clearHistory = () => {
    ...
    if (api?.resolveChat) {
      void api
        .resolveChat({ filePath: currentFilePath ?? null })
        .then((r) => {
          ...
          {showKbMenu && (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <strong style={{ fontSize: 12, color: 'var(--text-primary, #111)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                  <input
                    type="radio"
                    name="slides-kb-choice"
                    checked={!selectedKbId || selectedKbId === 'NONE'}
                    onChange={() => setSelectedKbId('NONE')}
                  />
                  <span>1. No Knowledge Base (Disabled)</span>
                </label>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                  <input
                    type="radio"
                    name="slides-kb-choice"
                    checked={selectedKbId === 'ALL'}
                    onChange={() => setSelectedKbId('ALL')}
                  />
                  <span>2. All Knowledge Bases</span>
                </label>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}>
                  <input
                    type="radio"
                    name="slides-kb-choice"
                    checked={Boolean(selectedKbId && selectedKbId !== 'ALL' && selectedKbId !== 'NONE')}
                    onChange={() => {
                      const first = effectiveKbList[0]
                      if (first) setSelectedKbId(first.id)
                    }}
                  />
                  <span>3. Specific Collection:</span>
                </label>
              ...
              <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    background: selectedKbId && selectedKbId !== 'NONE' ? 'var(--accent-subtle, #e6f0ff)' : undefined,
                    color: selectedKbId && selectedKbId !== 'NONE' ? 'var(--accent, #0066cc)' : undefined,
                  }}
                >
                  📚
                </button>
