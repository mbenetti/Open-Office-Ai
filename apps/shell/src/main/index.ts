...
/** app author's LinkedIn profile (opened from the welcome-page logo) */
const LINKEDIN_URL = 'https://www.linkedin.com/in/maurobenetti'

/** fork author's GitHub profile (opened from the settings → General acknowledgments) */
const GITHUB_URL = 'https://github.com/mbenetti'
...
function writeJson(p: string, data: unknown): void {
  try {
    ...
    console.error('Failed to write JSON:', p, err)
  }
}
...
function registerHomeIpc(): void {
  ...
  ipcMain.handle(HOME_CHANNELS.deleteFiles, async (_event, paths: unknown) => {
    ...
    for (const p of list) {
      ...
      // The document is gone — its per-document chat history goes with it.
      clearProjectChatForFile(p)
    }
  ...
  ipcMain.handle(HOME_CHANNELS.openLinkedIn, () => {
    shell.openExternal(LINKEDIN_URL).catch(() => {
      // no browser handler available; nothing actionable for the user here
    })
  })
...
async function exportPdfAsDocx(): Promise<void> {
  ...
  void dialog.showMessageBox(shellWindow, {
    type: 'info',
    message: 'PDF to Word conversion requires an active MCP Server configured in AI Settings.',
  })
}
