...
export interface ProjectApi {
  ...
  /** Creates a brand-new conversation for the file and makes it the current one */
  newChat(args: ResolveChatArgs): Promise<ResolveChatResult>
  /** Lists the file's conversations (newest first) with short previews, for the chat picker */
  listChats(args: ResolveChatArgs): Promise<ChatMeta[]>
  /** Switches the file's current chat to an existing conversation; null when the chat is unknown */
  selectChat(args: ResolveChatArgs & { chatId: string }): Promise<ResolveChatResult | null>
  ...
  /** Deletes a chat JSONL (discards that conversation for the document) */
  clearChat(args: { projectId: string; chatId: string }): Promise<void>
