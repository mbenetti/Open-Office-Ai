...
export interface ProjectIndex {
  ...
  /**
   * Absolute path → every chatId for the file (creation order). A document can
   * have several conversations; chatIdByPath points at the current one.
   * Old data without this map derives a single chat from chatIdByPath.
   */
  chatIdsByPath?: Record<string, string[]>
  /**
   * chatId → creation time (UTC ISO). The JSONL file is created lazily, so the
   * creation moment is recorded here when the chat is first registered.
   */
...
export interface ChatMeta {
  chatId: string
  /** Creation time of the conversation (UTC ISO), used as its display name */
  createdAt: string
  /** Last modification time of the JSONL file (UTC ISO) */
  updatedAt: string
  /** Approximate line count (estimated from file bytes, not exact) */
  approxCount: number
  /** First user message text (for the chat picker), truncated; absent for empty chats */
  preview?: string
}
