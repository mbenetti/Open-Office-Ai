...
describe('clearChat', () => {
  ...
  it('deletes the chat JSONL so history is gone', () => {
    store.appendChatMessage('default', 'chat-a', { role: 'user', text: 'hello' })
    store.appendChatMessage('default', 'chat-a', { role: 'assistant', text: 'hi' })
    expect(store.loadChat('default', 'chat-a')).toHaveLength(2)

    store.clearChat('default', 'chat-a')

    expect(store.loadChat('default', 'chat-a')).toHaveLength(0)
    expect(existsSync(join(tmpDir, 'projects', 'default', 'chats', 'chat-a.jsonl'))).toBe(false)
  })

  it('seq restarts from zero after clearing', () => {
    store.appendChatMessage('default', 'chat-a', { role: 'user', text: 'first' })
    store.appendChatMessage('default', 'chat-a', { role: 'assistant', text: 'reply' })
    store.clearChat('default', 'chat-a')

    store.appendChatMessage('default', 'chat-a', { role: 'user', text: 'fresh' })
    const msgs = store.loadChat('default', 'chat-a')
    expect(msgs).toHaveLength(1)
    expect(msgs[0]!.seq).toBe(0)
    expect(msgs[0]!.text).toBe('fresh')
  })

  it('is a no-op for a chat that does not exist', () => {
    expect(() => store.clearChat('default', 'missing')).not.toThrow()
  })
})

describe('clearChatForFile', () => {
  let tmpDir: string
  let store: ProjectStore
  ...
  it('deletes history for the file path and drops it from the project files list', () => {
    const filePath = '/tmp/some/document.pdf'
    const ids = store.resolveChatForFile(filePath)
    store.appendChatMessage(ids.projectId, ids.chatId, { role: 'user', text: 'q' })
    store.appendChatMessage(ids.projectId, ids.chatId, { role: 'assistant', text: 'a' })
    expect(store.loadChat(ids.projectId, ids.chatId)).toHaveLength(2)

    store.clearChatForFile(filePath)

    expect(store.loadChat(ids.projectId, ids.chatId)).toHaveLength(0)
    const proj = store.listProjectFiles(ids.projectId)
    expect(proj.includes(filePath)).toBe(false)
  })

  it('re-resolving the same path starts a fresh conversation', () => {
    const filePath = '/tmp/some/document.docx'
    const ids = store.resolveChatForFile(filePath)
    store.appendChatMessage(ids.projectId, ids.chatId, { role: 'user', text: 'old' })
    store.clearChatForFile(filePath)

    const again = store.resolveChatForFile(filePath)
    expect(again.chatId).toBe(ids.chatId)
    expect(store.loadChat(again.projectId, again.chatId)).toHaveLength(0)
  })
})

// ────────────────────────────────────────────────────────────
// 6c. multiple chats per document
// ────────────────────────────────────────────────────────────

describe('multiple chats per document', () => {
  let tmpDir: string
  let store: ProjectStore
  ...
  it('createChatForFile makes a new conversation current while keeping the old one', () => {
    const filePath = '/tmp/some/document.pptx'
    const first = store.resolveChatForFile(filePath)
    store.appendChatMessage(first.projectId, first.chatId, { role: 'user', text: 'first chat' })
    store.appendChatMessage(first.projectId, first.chatId, { role: 'assistant', text: 'reply 1' })

    const second = store.createChatForFile(filePath)
    expect(second.chatId).not.toBe(first.chatId)
    // The new chat is current and empty; the old one is still loadable
    expect(store.loadChat(second.projectId, second.chatId)).toHaveLength(0)
    expect(store.loadChat(first.projectId, first.chatId)).toHaveLength(2)
  })

  it('listChatsForFile returns all conversations newest first with previews', () => {
    const filePath = '/tmp/some/document.xlsx'
    const first = store.resolveChatForFile(filePath)
    store.appendChatMessage(first.projectId, first.chatId, { role: 'user', text: 'older question' })
    store.appendChatMessage(first.projectId, first.chatId, { role: 'assistant', text: 'older answer' })
    const second = store.createChatForFile(filePath)
    store.appendChatMessage(second.projectId, second.chatId, { role: 'user', text: 'newer question' })
    store.appendChatMessage(second.projectId, second.chatId, { role: 'assistant', text: 'newer answer' })

    const list = store.listChatsForFile(filePath)
    expect(list.map((c) => c.chatId)).toEqual([second.chatId, first.chatId])
    expect(list[0]!.preview).toBe('newer question')
    expect(list[1]!.preview).toBe('older question')
    // Each chat carries its creation timestamp (the chat-session display name),
    // recorded when the chat was created even before its JSONL file exists.
  ...
  it('selectChatForFile switches the current chat back to an older conversation', () => {
    const filePath = '/tmp/some/document.docx'
    const first = store.resolveChatForFile(filePath)
    store.appendChatMessage(first.projectId, first.chatId, { role: 'user', text: 'first' })
    const second = store.createChatForFile(filePath)
    expect(store.resolveChatForFile(filePath).chatId).toBe(second.chatId)

    const selected = store.selectChatForFile(filePath, first.chatId)
    expect(selected).toEqual(first)
    expect(store.resolveChatForFile(filePath).chatId).toBe(first.chatId)
  })

  it('selectChatForFile rejects a chat that does not belong to the file', () => {
    const filePath = '/tmp/some/document.docx'
    store.resolveChatForFile(filePath)
    expect(store.selectChatForFile(filePath, 'unknown-chat')).toBeNull()
  })

  it('clearChat removes only that conversation and keeps the others', () => {
    const filePath = '/tmp/some/document.docx'
    const first = store.resolveChatForFile(filePath)
    store.appendChatMessage(first.projectId, first.chatId, { role: 'user', text: 'first' })
    const second = store.createChatForFile(filePath)
    store.appendChatMessage(second.projectId, second.chatId, { role: 'user', text: 'second' })

    store.clearChat(first.projectId, first.chatId)

    expect(store.loadChat(first.projectId, first.chatId)).toHaveLength(0)
    expect(store.loadChat(second.projectId, second.chatId)).toHaveLength(1)
    // The current chat (second) stays current
    expect(store.resolveChatForFile(filePath).chatId).toBe(second.chatId)
    expect(store.listChatsForFile(filePath).map((c) => c.chatId)).toEqual([second.chatId])
  })

  it('clearing the current chat moves the current pointer to the remaining conversation', () => {
    const filePath = '/tmp/some/document.docx'
    const first = store.resolveChatForFile(filePath)
    store.appendChatMessage(first.projectId, first.chatId, { role: 'user', text: 'first' })
    const second = store.createChatForFile(filePath)
    ...
    // second is current; discard it → current should fall back to first
    store.clearChat(second.projectId, second.chatId)
    expect(store.resolveChatForFile(filePath).chatId).toBe(first.chatId)
  })

  it('clearing the only chat drops the current mapping; re-resolving creates a fresh one', () => {
    const filePath = '/tmp/some/document.docx'
    const ids = store.resolveChatForFile(filePath)
    store.appendChatMessage(ids.projectId, ids.chatId, { role: 'user', text: 'only' })
    store.clearChat(ids.projectId, ids.chatId)

    expect(store.listChatsForFile(filePath)).toHaveLength(0)
    const again = store.resolveChatForFile(filePath)
    expect(store.loadChat(again.projectId, again.chatId)).toHaveLength(0)
  })
})
