...
export function buildPdfFixture(text: string): Uint8Array {
  return buildPdfFixtureWithStream(`BT /F1 24 Tf 72 720 Td (${text}) Tj ET`)
}

/** same minimal pdf but with a caller-provided content stream (multi-line layouts) */
export function buildPdfFixtureWithStream(stream: string): Uint8Array {
  const bodies = [
